...
class EditProfilePictureController extends GetxController {
  ...
  Future<void> selectSingleImage({ImageSource? imageSource}) async {
    ...
    if (pickedImage != null) {
      ...
      AppUtility.printLog('Original file size: ${resultFile.lengthSync()} B');
      ...
      if (size > (5 * maxImageBytes)) {
        ...
        AppUtility.printLog('Result file size: ${resultFile.lengthSync()} B');
  ...
  Future<void> _removeProfilePicture() async {
    ...
    try {
      final response = await _apiProvider.deleteProfilePicture(_auth.authToken);
      final decodedData = jsonDecode(utf8.decode(response.bodyBytes));
